      ...
      while (!documentSelectSuccess) {
         try {
            ...
            if (!documentSelectSuccess) {
               JOptionPane.showMessageDialog(null, "The document does not exist.");
            }
         } catch (IOException e) {
            System.exit(1);
         } catch (ClassNotFoundException e) {
   ...
   private class characterListener implements KeyListener {
      ...
      public void keyReleased(KeyEvent e) {
         if (e.getKeyChar() > 47 && e.getKeyChar() < 125 && bold || underline || italic) {
            ...
            /// use mouse courser as the relative position so just add /b first
            // than string than b than move curser
            // according to counter.

            // take two strings, one with <b> tags and one without. Than add
            // element to one without, go through loop to parse it with the
            // tags, update cursor position, than set text
            ...
            // corner case, if user clicks very end of document text, it will
            // fix the alighning issues
            if (textArea.getCaretPosition() > noTagString.length()) {
               textArea.setCaretPosition(noTagString.length() + 1);
            }

            // wierd corner case with inital caret positions. Done to print
            // current key typed in the
            // right format to be pasted into the right location of the noHTML
            // string
            ...
            // System.out.println("This is noTagString: "+
            // noTagString.toString());

            // sets the String with tags to only consis of letters and b,u,i
            // tags
            ...
            // parse the two strings together with carrot Position being taken
            // into effect
            // should only increase when a non tag is inserted
            int increment = 0; // increment will only increase when a key(not a
                               // tag) is copied
            int stopper = stringWithTags.length();
            // while(i<stopper){
            for (int i = 0; i < stringWithTags.length(); i++) {
               // System.out.println( "increment" + increment + "carrot "+
               // carrotPosition+ "string length: " + stringWithTags.length());
               ...
               // idea is to insert string into String with tags when
               // increment=carrot
               ...
               if (increment == carrotPosition) {
                  ...
                  // Another idea is to Break and just copy the rest of the
                  // string since it's going to be the same...
               }
               // inserts tags into string with no tags
               // while < is read (this only does one character at a time...
               // inefficient...
               while ((stringWithTags.charAt(i) + "").equals("<")) {
                  ...
                  // this if says that if the thing after < is not equal do the
                  // loop
                        ...
                        while (!(stringWithTags.charAt(i) + "").equals("<")) {
                           ...
                           if (i + 1 > stringWithTags.length() - 1) {
                              ...
                              while (stringWithTags.length() > noTagString.length()) {
                                 ...
                                 currentLocation++;
                                 // System.out.println("final noTagString: " +
                                 // noTagString.toString());
                              }
                           ...
                           increment++;
                           // if inserting in middle of bold or ita, or under,
                           // insert.
                           // This loop fixes inserting in middle but causes it
                           // to never become unbold or ui or uu...
                           if (increment == carrotPosition) {
                              // here search for end tags before inserting text
                              // than insert begining tags
                              ...
                              // Another idea is to Break and just copy the rest
                              // of the string since it's going to be the
                              // same...
                           }
                        }
                        // to here. However without it, it acts correctlyish
                        // with each button, but cant edit from
                        // middle.
